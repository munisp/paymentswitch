...
import { users, previewSessions } from "../drizzle/schema";
...
export const appRouter = router({
  ...
  rateAlerts: router({
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        return await rateAlertService.deleteRateAlert(input.alertId, ctx.user.id);
      }),
  ...
  merchant: router({
    ...
    create: protectedProcedure
      .input(z.object({
        ...
        businessType: z.enum(["ecommerce", "saas", "marketplace", "nonprofit", "other"]),
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (database) {
          await database.update(users).set({ role: 'merchant' }).where(eq(users.id, ctx.user.id));
        }
  ...
  preview: router({
    getSession: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        if (result.length === 0) {
          throw new TRPCError({ code: 'NOT_FOUND', message: 'Preview session not found' });
        }
        ...
        if (new Date() > session.expiresAt) {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Preview session expired' });
        }
  ...
  payment: router({
    ...
    createSession: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (merchant.status !== 'active') {
          throw new TRPCError({ code: 'FORBIDDEN', message: 'Merchant account not active' });
        }
    ...
    getSession: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        // Check if expired
        if (new Date() > session.expiresAt) {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Payment session expired' });
        }
    ...
    processPayment: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (session.status !== 'pending') {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Payment session already processed' });
        }
        ...
        try {
          ...
          const activeWebhook = webhooks.find((w: any) => w.enabled && w.events.includes('payment.completed'));
          
          if (activeWebhook) {
            await sendWebhook(
              activeWebhook.id,
              transaction.status === 'captured' ? 'payment.completed' : 'payment.failed',
              {
                sessionId: input.sessionId,
        ...
        return {
          transactionId: transaction.transactionId,
          status: transaction.status,
          redirectUrl: transaction.status === 'captured' ? session.successUrl : session.cancelUrl,
        };
      }),
    ...
    getTransaction: publicProcedure
      ...
      .query(async ({ input }) => {
        const transaction = await db.getTransactionByTransactionId(input.transactionId);
        if (!transaction) {
    ...
    create: merchantProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (transaction.status !== 'captured') {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Can only refund captured transactions' });
        }

        // Calculate refund amount
        const existingRefunds = await db.getRefundsByTransaction(input.transactionId);
        const totalRefunded = existingRefunds.reduce((sum, r) => 
          r.status === 'completed' ? sum + r.amount : sum, 0
        );
        const refundAmount = input.amount || (transaction.amount - totalRefunded);

        if (refundAmount > transaction.amount - totalRefunded) {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Refund amount exceeds available balance' });
        }
        ...
        if (newTotalRefunded >= transaction.amount) {
          await db.updateTransaction(input.transactionId, { status: 'refunded' });
        } else {
          await db.updateTransaction(input.transactionId, { status: 'partially_refunded' });
        }
