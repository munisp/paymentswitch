...
type ProbeResult<T> = { ok: true; data: T } | { ok: false; error: string };
...
function statusFromProbe<T>(
  ...
  if (!result.ok) {
    return {
      service,
      status: result.error.includes("not configured")
        ? "misconfigured"
        : "unavailable",
      checkedAt,
      details,
...
const TIGERBEETLE_ADDRESSES =
  process.env.TIGERBEETLE_ADDRESSES ?? process.env.TIGERBEETLE_ADDRESS;

async function probeTcp(address: string): Promise<ProbeResult<null>> {
  const [host, portText] = address.trim().split(":");
  const port = Number.parseInt(portText ?? "", 10);
  if (!host || !Number.isInteger(port) || port < 1 || port > 65535) {
    return { ok: false, error: `Invalid TigerBeetle address: ${address}` };
  }
  return new Promise(resolve => {
    const socket = net.createConnection({ host, port });
    const timer = setTimeout(() => {
      socket.destroy();
      resolve({ ok: false, error: `TCP probe timed out for ${address}` });
    }, TIMEOUT_MS);
    socket.once("connect", () => {
      clearTimeout(timer);
      socket.end();
...
export async function getKafkaLiveStatus(): Promise<InfrastructureStatus> {
  if (!KAFKA_REST_URL || !SCHEMA_REGISTRY_URL) {
    return statusFromProbe("Kafka", {
      ok: false,
      error: "KAFKA_REST_URL and SCHEMA_REGISTRY_URL are required",
    });
  }

  const [brokers, subjects] = await Promise.all([
    infraFetch<{ brokers?: unknown[] }>(
      `${KAFKA_REST_URL.replace(/\/$/, "")}/brokers`
    ),
  ...
  if (!brokers.ok || !subjects.ok) {
    return statusFromProbe("Kafka", {
      ok: false,
      error: [brokers, subjects]
        .filter((probe): probe is { ok: false; error: string } => !probe.ok)
        .map(probe => probe.error)
        .join("; "),
    });
  }

  return statusFromProbe(
    "Kafka",
    { ok: true, data: null },
    {
...
export async function getRedisLiveStatus(): Promise<InfrastructureStatus> {
  return probeJson<string>(
    "Redis",
    REDIS_EXPORTER_URL
      ? `${REDIS_EXPORTER_URL.replace(/\/$/, "")}/metrics`
      : undefined,
    "REDIS_EXPORTER_URL",
    () => ({ exporter: "reachable" })
  );
...
export async function getPostgresLiveStatus(): Promise<InfrastructureStatus> {
  if (!PATRONI_URL && !PGBOUNCER_URL) {
    return statusFromProbe("PostgreSQL", {
      ok: false,
      error: "PATRONI_URL or PGBOUNCER_EXPORTER_URL is required",
    });
  }

  const probes = await Promise.all([
    PATRONI_URL
      ? infraFetch<unknown>(`${PATRONI_URL.replace(/\/$/, "")}/cluster`)
      : null,
    PGBOUNCER_URL
      ? infraFetch<string>(`${PGBOUNCER_URL.replace(/\/$/, "")}/metrics`)
      : null,
  ]);
  const successes = probes.filter(
    (probe): probe is { ok: true; data: unknown } =>
  ...
  if (successes.length === 0) {
    return statusFromProbe("PostgreSQL", {
      ...
      error:
        probes
          .filter(
            (probe): probe is { ok: false; error: string } =>
              probe !== null && !probe.ok
          )
          .map(probe => probe.error)
          .join("; ") || "No PostgreSQL health endpoint responded",
    });
  }
  return statusFromProbe(
    "PostgreSQL",
...
export async function getTigerBeetleLiveStatus(): Promise<InfrastructureStatus> {
  const addresses = (TIGERBEETLE_ADDRESSES ?? "")
    .split(",")
    .map(value => value.trim())
    .filter(Boolean);
  if (addresses.length === 0) {
    return statusFromProbe("TigerBeetle", {
      ok: false,
      error: "TIGERBEETLE_ADDRESSES or TIGERBEETLE_ADDRESS is required",
    });
  }
  const probes = await Promise.all(addresses.map(probeTcp));
  const reachableAddresses = addresses.filter(
    (_, index) => probes[index]?.ok === true
  );
  if (reachableAddresses.length === 0) {
    return statusFromProbe(
      "TigerBeetle",
      {
        ...
        error:
          probes
            .map(probe => (probe.ok ? "" : probe.error))
            .filter(Boolean)
            .join("; ") || "No TigerBeetle replica accepted a TCP connection",
      },
      { configuredReplicaCount: addresses.length }
    );
  }
  return statusFromProbe(
    "TigerBeetle",
    ...
    {
      configuredReplicaCount: addresses.length,
      reachableReplicaCount: reachableAddresses.length,
      reachableAddresses,
    }
  );
}
...
export async function getKeycloakLiveStatus(): Promise<InfrastructureStatus> {
  if (!KEYCLOAK_URL || !KEYCLOAK_REALM) {
    return statusFromProbe("Keycloak", {
      ok: false,
      error: "KEYCLOAK_URL and KEYCLOAK_REALM are required",
    });
  }
  ...
  const [health, discovery] = await Promise.all([
    ...
    infraFetch<{ issuer?: string }>(
      `${baseUrl}/realms/${encodeURIComponent(KEYCLOAK_REALM)}/.well-known/openid-configuration`
    ),
  ]);
  if (
    !health.ok ||
    ...
    !discovery.data.issuer
  ) {
    const errors = [
      !health.ok
        ? health.error
        : health.data.status !== "UP"
          ? "Keycloak health endpoint is not UP"
          : "",
      !discovery.ok
        ? discovery.error
        : !discovery.data.issuer
          ? "OIDC discovery document has no issuer"
          : "",
    ].filter(Boolean);
  ...
  return statusFromProbe(
    "Keycloak",
    { ok: true, data: null },
    { realm: KEYCLOAK_REALM, issuer: discovery.data.issuer }
  );
}

export async function getApisixLiveStatus(): Promise<InfrastructureStatus> {
  if (!APISIX_ADMIN_URL || !APISIX_ADMIN_KEY) {
    return statusFromProbe("APISIX", {
      ok: false,
      error: "APISIX_ADMIN_URL and APISIX_ADMIN_KEY are required",
    });
  }
  const result = await infraFetch<{ list?: unknown[]; total?: number }>(
    `${APISIX_ADMIN_URL.replace(/\/$/, "")}/apisix/admin/routes`,
    { headers: { "X-API-KEY": APISIX_ADMIN_KEY } }
  );
  return result.ok
    ? statusFromProbe("APISIX", result, {
        routeCount: result.data.list?.length ?? result.data.total ?? 0,
      })
    : statusFromProbe("APISIX", result);
}

export async function getPermifyLiveStatus(): Promise<InfrastructureStatus> {
  return probeJson<unknown>(
    "Permify",
    PERMIFY_URL ? `${PERMIFY_URL.replace(/\/$/, "")}/healthz` : undefined,
    "PERMIFY_URL",
    () => ({ healthEndpoint: "reachable" })
  );
}

export async function getDaprLiveStatus(): Promise<InfrastructureStatus> {
  if (!DAPR_HTTP_PORT)
    return statusFromProbe("Dapr", {
      ok: false,
      error: "DAPR_HTTP_PORT is required",
    });
  const result = await infraFetch<{
    id?: string;
    components?: unknown[];
    subscriptions?: unknown[];
  }>(`http://127.0.0.1:${DAPR_HTTP_PORT}/v1.0/metadata`);
  return result.ok
    ? statusFromProbe("Dapr", result, {
        appId: result.data.id ?? null,
        activeComponents: result.data.components?.length ?? 0,
        subscriptions: result.data.subscriptions?.length ?? 0,
      })
    : statusFromProbe("Dapr", result);
}
...
export async function getOpenSearchLiveStatus(): Promise<InfrastructureStatus> {
  return probeJson<{
    ...
    "OpenSearch",
    OPENSEARCH_URL
      ? `${OPENSEARCH_URL.replace(/\/$/, "")}/_cluster/health`
      : undefined,
    "OPENSEARCH_URL",
    data => ({
      clusterName: data.cluster_name ?? null,
      clusterStatus: data.status ?? null,
      nodeCount: data.number_of_nodes ?? 0,
    })
  );
}
...
export async function getFluvioLiveStatus(): Promise<InfrastructureStatus> {
  return probeJson<unknown>(
    "Fluvio",
    FLUVIO_URL ? `${FLUVIO_URL.replace(/\/$/, "")}/api/v1/status` : undefined,
    "FLUVIO_URL",
...
export async function getOpenAppSecLiveStatus(): Promise<InfrastructureStatus> {
  return probeJson<unknown>(
    "OpenAppSec",
    OPENAPPSEC_URL
      ? `${OPENAPPSEC_URL.replace(/\/$/, "")}/api/v1/status`
      : undefined,
    "OPENAPPSEC_URL",
...
export async function getMojaloopLiveStatus(): Promise<InfrastructureStatus> {
  return probeJson<unknown>(
    "Mojaloop",
    MOJALOOP_URL ? `${MOJALOOP_URL.replace(/\/$/, "")}/health` : undefined,
    "MOJALOOP_URL",
    () => ({ healthEndpoint: "reachable" })
  );
}
...
export async function getObservabilityLiveStatus(): Promise<InfrastructureStatus> {
  return probeJson<string>(
    "Observability",
    PROMETHEUS_URL ? `${PROMETHEUS_URL.replace(/\/$/, "")}/-/ready` : undefined,
    "PROMETHEUS_URL",
