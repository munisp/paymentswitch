...
async def health_check():
    ...
    if _shutting_down:
        return JSONResponse(status_code=503, content={"status": "shutting_down", "service": "notification-service"})
...
async def _shutdown(sig):
    ...
    global _shutting_down
    _shutting_down = True
    logger.info(f"Received {sig.name}, shutting down notification-service gracefully")
