...
async def health_check():
    ...
    if _shutting_down:
        return JSONResponse(status_code=503, content={"status": "shutting_down", "service": "vpa-service"})
...
async def shutdown_event():
    ...
    _shutting_down = True
    logger.info("vpa-service shutting down gracefully")

async def _shutdown(sig):
    """Handle shutdown signal."""
    global _shutting_down
    _shutting_down = True
    logger.info(f"Received {sig.name}, shutting down vpa-service gracefully")
